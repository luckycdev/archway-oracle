...
def calculate_daily_traffic(df, segment_id, target_date):
    """Sums the vehicle count for the 24 hours of the selected day."""
    # This check ensures we handle both datetime and date objects safely
    if hasattr(target_date, 'date'):
        target_date_only = target_date.date()
    else:
        target_date_only = target_date

    day_data = df[(df['road_segment_id'] == segment_id) & 
...
def calculate_aadt_estimate(daily_total, target_date):
    """
    Estimates Annual Average Daily Traffic.
    Formula: (Current Daily Volume / Monthly Factor)
    """
    month = target_date.month
    factor = MONTHLY_FACTORS.get(month, 1.0)
    
    # If it's a busy month (factor > 1), the AADT (average) 
    # will be lower than today's high volume.
